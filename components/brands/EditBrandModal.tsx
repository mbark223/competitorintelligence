...
export default function EditBrandModal({ isOpen, brand, onClose, onSuccess }: EditBrandModalProps) {
  ...
  // Pre-fill form when brand changes
  useEffect(() => {
    if (brand) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/brands/${brand.recordId}`, {
        method: 'PUT',
        headers: {
